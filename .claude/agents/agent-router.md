...
FE / BE / INFRA / CICD / SRE（監視設定コード）へのディスパッチ時、dispatch brief に以下を必ず含めること:

- `task_id`: PM の `TASK-XXX`
- `implementation_spec`: 実装仕様の要点（Codex プロンプトのベースとなる内容）
- `design_docs`: 参照すべき設計ドキュメントのパス一覧

実装 Agent は設計ドキュメントを読み込んで Codex プロンプトを構成し、`codex:codex-rescue` サブエージェント（`isolation: "worktree"`）に委任する。ローカルへの直接書き込みは行わない。
