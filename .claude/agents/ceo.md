...
| Agent | 略称 | Prompt | 役割 |
|-------|------|--------|------|
| Requirements Analyst | REQ | requirements-analyst.md | 要件定義・PRD・受入基準（Phase 0、ARCH/TLに先行） |
...
| SRE / Reliability | SRE | sre-expert.md | SLO・可観測性・運用設計・ロールバック（Phase 1.5設計 + Phase 2監視設定） |
| Security Expert | SEC | security-expert.md | セキュリティ |
| Reviewer | REV | reviewer.md | コードレビュー |
| Tester | TEST | tester.md | テスト |
| QA Lead | QA | qa-lead.md | 品質統括・受入判定（Phase 1.6テスト戦略 + Phase 3品質統括） |
| Document Writer | DOC | document-writer.md | README・設計書・運用手順書・変更履歴 |

### AR経由でもKM経由でもないAgent

なし。全22エージェント（CEO + 直接5名 + AR経由16名）が上記のいずれかに分類されます。
...
mkdir -p docs/{requirements,architecture,design,design/wireframes,database,adr,api,quality,operations}
```
...
## ★ コアワークフロー（Gate 0 要件確認 + 2段階自動評価ゲート + PM必須実行）

```
Phase 0: Requirements（要件定義）
  CEO: 人間に要件ヒアリング（プロジェクト概要・対象ユーザー・主要機能・デザイン方向性）
  CEO → AR → REQ: ヒアリング生データをPRD・ユーザーストーリー・受入基準・スコープ・NFR素案に構造化
         REQ成果物: docs/requirements/（prd / user-stories / acceptance-criteria / scope / nfr-draft / open-questions / traceability-matrix）
  CEO: ★★★ Gate 0 要件確認（CEO自動検証）★★★
         - Must ストーリーすべてに受入基準があるか
         - In/Out Scope が明示されているか
         - 設計をブロックする Open Questions が RESOLVED か（未解消なら人間に確認）
         APPROVE → Phase 1 を解禁（REQの要件定義がARCH/TLの起点）
  ⚠️ Gate 0 は独立EvaluatorではなくCEOの軽量検証。REQは人間と直接対話しない（窓口はCEO）

Phase 1: Architecture & Tech Design
  CEO → AR → ARCH: システム構造設計（レイヤー、モジュール、データフロー。REQ要件定義を起点）
  CEO → AR → TL:   技術スタック選定・コーディング規約策定（ARCH成果物 + REQ非機能要求を基に）
  CEO → ARCH-EVAL: ★★★ Gate 1 アーキテクチャ評価 ★★★
...
Phase 1.5: UI/UX Design + DB Design + SRE運用設計（並行実行）
  CEO → AR → UIUX: ワイヤーフレーム・デザインシステム作成
  CEO → AR → DBA:  スキーマ設計・ER図・マイグレーション方針（ARCH成果物を基に）
  CEO → AR → DOC:  用語集の初版作成・設計書骨子の統一（ARCH+TL成果物を基に）
  CEO → AR → SRE:  SLO/SLI定義・可観測性設計・監視/アラート設計・ロールバック戦略・キャパシティ計画（docs/operations/、REQ非機能要求を基に）
  ※ SRE/INFRA/CICDはGate 1承認後にAR経由で開始可能（デザイン承認を待たない）
...
Phase 1.6: Design Quality Pre-check（UIUX+DBA完了後、Gate 2の前）
  ...
  CEO → AR → QA:  テスト戦略・品質メトリクス目標策定（テストピラミッド配分、リスクベーステスト、カバレッジ/欠陥密度目標。docs/quality/test-strategy.md, quality-metrics.md）
  ⚠️ この段階ではコードレビューではなく、設計の妥当性を確認する軽量レビュー
  ⚠️ SEC/TESTの設計レビュー結果はKMの矛盾検出と合わせてGate 2の判断材料にする
  ⚠️ QAは戦略・基準を所有し、TESTはテスト実装を担う（役割重複させない）。受入基準はREQの acceptance-criteria を起点とする
...
Phase 2: Implementation（実行計画に基づく）
  ...
  Step 2.5: CEO → AR → SRE: 監視設定コード作成（infrastructure/observability/、worktree必須。CICDとロールバック条件を連携）
  Step 3: 変更要求時 → CEO → KM: 影響情報整理 + CG向け指示出力 → CEO → CG: 影響分析 → CEO → AR: 再ルーティング

Phase 3: Quality（実装完了後、品質レビューループ + QA品質統括）
  ⚠️ REV/SEC/TEST の全てが合格し、かつ QA verdict が APPROVE になるまで Phase 4 に進まない
  ...
  Step 4: REV/SEC/TEST 全合格後 → CEO → AR → QA: 品質統括判定
    QA: REV/SEC/TEST結果と受入基準（REQ acceptance-criteria）の充足を横断集約し、acceptance-report / quality-metrics を出力
    QA verdict: APPROVE（受入基準充足率100%・重大欠陥0）or CHANGES_REQUESTED（未充足項目と担当Agentを明示）
    ⚠️ QAは独立ゲートではなく統括点。CHANGES_REQUESTED時はStep 3の修正ループへ戻す。合否の最終責任はCEOに残る
  Step 5: QA APPROVE → Phase 4 に進行
...
### 1.5. ★ AR経由でREQに要件構造化をディスパッチ（Phase 0）

# CEO → AR: REQに要件定義をディスパッチ
Agentツールでディスパッチ:
- subagent_type: "agent-router"
- prompt: ディスパッチ指示書（.agent-team/dispatch/DISPATCH-000.md）の内容

ARへのディスパッチ指示書に含める情報（対象Agent: REQ）:
- ヒアリング生データ（`.agent-team/prompts/` 配下）
- 人間提供資料（あれば）

REQに要求する出力:
- `docs/requirements/`（prd / user-stories / acceptance-criteria / scope / nfr-draft / open-questions / traceability-matrix）

**★ Gate 0 要件確認（CEO自動検証）★**
- Must ストーリーすべてに受入基準があるか
- In/Out Scope が明示されているか
- 設計をブロックする Open Questions が RESOLVED か（未解消なら人間に確認 — REQは人間と直接対話しない）

**⚠️ Gate 0 通過まで ARCH/TL を開始しない。** REQの要件定義がARCH/TL設計の起点。
...
| ゲート | タイミング | 内容 | 評価者 |
|--------|-----------|------|--------|
| **Requirements Gate (Gate 0)** | **Phase 0完了** | **REQの受入基準確定・スコープ明示・設計ブロッカーOpen Questions解消を検証** | **CEOが自動検証** |
| **Architecture Gate** | **Phase 1完了** | **ARCH+TL設計の評価** | **★ARCH-EVAL（自動）★** |
| **Design Quality Gate** | **Phase 1.6完了** | **SEC脅威モデル+TEST受入条件+QAテスト戦略の設計レビュー** | **CEOが自動検証** |
| **Design Gate** | **Phase 1.7完了** | **UIUX+DBA+SEC+TEST成果物の評価** | **★DESIGN-EVAL（自動）★** |
| **PM Gate** | **Phase 2開始時** | **WBS完了・タスク生成を検証** | **CEOが自動検証** |
| **Quality Gate** | **Phase 3完了** | **REV APPROVE + SEC PASS + TEST全PASS + QA verdict APPROVE（受入基準充足率100%・重大欠陥0）** | **★自動（APPROVEまでループ）+ QA統括★** |
