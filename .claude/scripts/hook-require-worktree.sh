#!/usr/bin/env bash
# PreToolUse hook: FE/BE/INFRA/CICD/SRE が実装成果物を書き込む際、
# 現在の作業ディレクトリが git worktree（メインツリー以外）であることを要求する。
#
# - 対象ツール: Write / Edit / NotebookEdit（matcher 側で絞る想定）
# - 対象パス: frontend/ backend/ infrastructure/（observability/ 含む） .github/workflows/ tests/
...
hook-require-worktree: 実装成果物の編集はメインツリーでは禁止されています。
  対象ファイル: $FILE_PATHS
  ルール: FE/BE/INFRA/CICD/SRE は実装着手前に git worktree を作成し、その中で編集してください。
  例:
