...
**Agent ツールで orchestrator subagent を 1 つ spawn する。`model: "opus"` を指定すること。**

subagent への prompt テンプレート:
...
**session_id** には UserPromptSubmit hook が注入した `EVAL_LOOP_SESSION_ID` の値を使う。
